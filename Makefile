...
FIXTURES_DIR := $(TEST_DIR)/integration/fixtures/db_layer
BUILD_DIR := build
...
#                     実ファイルI/O・cJSONパース・iconv変換を経由するIntegration Test的な位置づけのため
#                     test/integration/ 配下に置く（design_db_layer.md 7, 9節参照）。
TEST_DB_LAYER_SRCS := $(TEST_DIR)/integration/test_db_layer.c \
                       $(SRC_DIR)/db_layer.c \
                       $(CJSON_DIR)/cJSON.c \
                       $(UNITY_DIR)/unity.c
